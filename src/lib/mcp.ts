...
export async function getMcpServer() {
  if (isInitialized && mcpServer && mcpTransport) {
    return { mcpServer, mcpTransport };
  }

  mcpServer = new Server({
    name: "squirryfy-discovery-mcp",
    version: "1.0.0"
  }, {
    capabilities: { tools: {} }
  });
  ...
  mcpServer.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "create_instagram_post") {
      ...
      if (error) {
        console.warn(`[MCP create_instagram_post] Warning: Database insert failed (${error.message}). Returning mock post ID for local testing/verification.`);
    ...
    if (name === "update_instagram_post") {
      ...
      if (error) {
        console.warn(`[MCP update_instagram_post] Warning: Database update failed (${error.message}). Mocking success.`);
    ...
    if (name === "upload_slide_image") {
      ...
      try {
        ...
        console.warn(`[Supabase Storage] Bucket check/create skipped/failed: ${e?.message || e}`);
      }
  ...
  // Bind the server to the web standard streamable transport
